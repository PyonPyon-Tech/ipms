...
export const InventoryEditStock: FC<Pesticide> = ({ id, name, activeIngredient, stock, targets, unit, targetPests }) => {
  ...
  const onSubmit = async (data: any) => {
    ...
    AxiosClient.put(`${URL_INVENTORY}/pesticides/${router.query.id}`, submittedData)
      .then((response) => {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="flex">
          ...
          <div className="my-2 flex w-1/2 flex-col gap-2 rounded-r-[8px] bg-teal-light p-4 align-middle">
            ...
            <input className="pb-1" type="number" {...register("stock")} />
          </div>
        </div>
        <button
          type="submit"
