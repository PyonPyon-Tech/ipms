...
import {
  OutletFields,
  OutletFormFactory,
} from "@models/customer/outlet/form";
...
export const OutletForm: FC<{ }> = ({  
  ...
  const onSubmit = async (data: OutletFields) => {
    ...
    AxiosClient.post(
      `${URL_CUSTOMER}/${customerId}/outlets`,
      outlet
    )
      .then((response) => {
    ...
    <Container className="justify-evenly gap-x-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="detail-form">
        ...
        <select {...register("supervisorId")}>
            {supervisors.map((supervisor) => <option value={supervisor.id} key={supervisor.id}>{supervisor.user.name}</option>)}
        </select>
