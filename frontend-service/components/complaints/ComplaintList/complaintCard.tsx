...
export const ComplaintCard: FC<Complaint> = ({
  ...
  return (
    ...
    >
        ...
        />
        <div className="">
          <h5 className="font-bold text-sm md:text-xl md:mb-1">{ROLES[user?.role ?? 0] == "Customer" ? (report == null ? `Komplain Umum ${outletName}` : `Komplain Kunjungan ${report.outlet.name}`) : customerUser.name}</h5>
          <div>
              ...
              <tbody>
                <tr>
                  {!report && <>
                    ...
                    {ROLES[user?.role ?? 0] != "Customer" && <td className="pr-4">Outlet: {`${outletName}`}</td>}
                  </>}
                  {!!report && <>
      ...
      {!!isAcknowledged && <Tag key={id + "tag"} title={"Diproses"} className={"bg-teal"} ></Tag>}
      {!isAcknowledged && <Tag key={id + "tag"} title={"Belum Diproses"} className={"bg-coral"} ></Tag>}
    </div>
  );
};
