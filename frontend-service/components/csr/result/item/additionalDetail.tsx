  ...
  useEffect(() => {
    ...
    async function retrieveImageDatas(imageUrls:String[]) {
      for (let i = 0; i < imageUrls.length; i++) {
        AxiosClient.get(`${URL_IMAGE}/${imageUrls[i]}`)
        .then((response) => {
          let imageUrlObj = new String(response.data);
  ...
  return recommendations.length != 0 ? (
    <div className="mb-2 w-0 min-w-full max-w-full">
      <div
        ...
        className="my-2 flex w-full gap-2 flex-wrap flex-row items-start justify-center rounded-md border-2 border-solid border-[rgba(0,0,0)] bg-gray-200 p-2"
      >
        {imageUrls.length != 0 ? (
          imageUrls.map((item) => {
            return (
